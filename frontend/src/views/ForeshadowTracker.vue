    ...
    <div class="mx-auto max-w-6xl px-6 py-10">
      <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <router-link
            :to="`/novels/${novelId}`"
            class="group mb-3 inline-flex items-center gap-1.5 text-sm font-semibold text-vermilion-600 hover:text-vermilion-700"
          >
          ...
          <h1 class="text-3xl font-extrabold tracking-tight text-ink-900">伏笔追踪</h1>
          <p class="mt-2 text-sm text-ink-500">追踪已种下、已回收和悬而未解的故事线索。</p>
        </div>

        <button
          class="inline-flex items-center justify-center rounded-xl border border-ink-200 bg-white px-4 py-2 text-sm font-semibold text-ink-700 shadow-sm transition hover:border-ink-300 hover:bg-paper-100 disabled:cursor-not-allowed disabled:opacity-60"
          type="button"
          :disabled="loading"
      ...
      <div v-if="loading" class="rounded-xl border border-ink-200 bg-white p-12 text-center shadow-sm">
        <div class="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-vermilion-600 border-t-transparent"></div>
        <p class="mt-4 text-sm font-medium text-ink-500">正在加载伏笔数据...</p>
      </div>
      ...
      <template v-else>
        <div class="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div class="rounded-xl border border-ink-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <p class="text-xs font-bold uppercase tracking-wider text-ink-400">总计</p>
            <p class="mt-3 text-4xl font-extrabold text-ink-900">{{ totalCount }}</p>
            <p class="mt-1 text-xs text-ink-500">追踪中的伏笔</p>
          </div>

          <div class="rounded-xl border border-ink-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <p class="text-xs font-bold uppercase tracking-wider text-ink-400">已回收</p>
            <p class="mt-3 text-4xl font-extrabold text-blue-600">{{ resolvedCount }}</p>
            <p class="mt-1 text-xs text-ink-500">已经完成回收</p>
          </div>

          <div class="rounded-xl border border-ink-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <p class="text-xs font-bold uppercase tracking-wider text-ink-400">悬挂</p>
            <p class="mt-3 text-4xl font-extrabold text-red-600">{{ danglingCount }}</p>
            <p class="mt-1 text-xs text-ink-500">需要关注</p>
          </div>

          <div class="flex items-center justify-between rounded-xl border border-ink-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            <div>
              <p class="text-xs font-bold uppercase tracking-wider text-ink-400">回收率</p>
              <p class="mt-3 text-4xl font-extrabold text-ink-900">{{ resolutionPercent }}%</p>
            </div>
            <div class="relative h-20 w-20">
              <svg class="h-20 w-20 -rotate-90" viewBox="0 0 80 80" aria-hidden="true">
                <circle cx="40" cy="40" r="32" fill="none" stroke="currentColor" stroke-width="8" class="text-paper-100" />
                <circle
                  cx="40"
                  cy="40"
                  r="32"
                  ...
                  class="text-vermilion-600 transition-all duration-700"
                  :stroke-dasharray="progressCircumference"
                  :stroke-dashoffset="progressOffset"
                />
              </svg>
              <span class="absolute inset-0 flex items-center justify-center text-sm font-extrabold text-vermilion-700">{{ resolutionPercent }}%</span>
            </div>
          </div>
        </div>

        <div v-if="totalCount === 0" class="rounded-xl border border-ink-200 bg-white p-12 text-center shadow-sm">
          <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-xl bg-paper-100 text-ink-500">
          ...
          <h2 class="mt-5 text-base font-bold text-ink-900">暂未检测到伏笔</h2>
          <p class="mt-2 text-sm text-ink-500">生成更多章节后，系统会自动追踪已种下和未回收的线索。</p>
        </div>

        <template v-else>
          <div class="mb-6 rounded-xl border border-ink-200 bg-white px-4 pt-4 shadow-sm">
            <nav class="flex gap-3 overflow-x-auto pb-4" role="tablist">
              <button
                ...
                :class="activeTab === tab.id ? tab.activeClass : 'border-ink-200 bg-white text-ink-500 hover:bg-paper-50'"
          ...
          <div v-if="currentList.length === 0" class="rounded-xl border border-ink-200 bg-white p-10 text-center shadow-sm">
            <h2 class="text-base font-bold text-ink-900">暂无{{ activeTabLabel }}伏笔</h2>
            <p class="mt-2 text-sm text-ink-500">该分类当前为空。</p>
          </div>
          ...
          <div v-else class="space-y-4">
            ...
            >
              <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h2 class="text-base font-extrabold text-ink-900">{{ item.name || '未命名伏笔' }}</h2>
                  <p class="mt-2 max-w-3xl text-sm leading-6 text-ink-500">{{ item.description || '暂无描述。' }}</p>
                </div>
              ...
              <div class="mt-5 flex flex-wrap items-center gap-3 text-sm font-semibold">
                <span class="rounded-lg border border-ink-200 bg-paper-50 px-3 py-1.5 text-ink-700">
                  种下：第 {{ chapterLabel(item.planted_chapter) }} 章
                </span>
                <svg class="h-4 w-4 text-ink-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true">
              ...
              <div class="mt-5">
                <div class="mb-2 flex justify-between text-xs font-semibold text-ink-400">
                  <span>第 1 章</span>
                  <span>时间线</span>
                  <span>第 {{ maxChapter }} 章</span>
                </div>
                <div class="relative h-2 overflow-hidden rounded-full bg-paper-100">
...
const currentCardClass = computed(() => {
  if (activeTab.value === 'resolved') return 'border-l-4 border-l-blue-500 border-ink-200 hover:border-blue-200'
  if (activeTab.value === 'dangling') return 'border-l-4 border-l-red-500 border-ink-200 hover:border-red-200'
  return 'border-l-4 border-l-green-500 border-ink-200 hover:border-green-200'
})
