<template>
  <div class="max-w-3xl mx-auto px-6 py-10">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-xl font-bold text-neutral-900">创作对话</h1>
        <p class="text-sm text-neutral-500">{{ conversation?.topic }}</p>
      </div>
    ...
    <div class="space-y-4 mb-6 max-h-[500px] overflow-y-auto" ref="messagesContainer">
      <div v-for="msg in messages" :key="msg.id"
        :class="['p-4 rounded-lg max-w-[85%]', msg.role === 'user' ? 'ml-auto bg-accent-50 text-neutral-900' : 'bg-white border border-neutral-200']"
      >
        ...
        <div class="flex items-center gap-2 mt-2">
          ...
          <template v-if="msg.role === 'assistant' && !msg.confirmed_as && conversation?.status === 'active'">
            <button @click="confirmMsg(msg.id, 'world')" class="text-[10px] text-accent-500 hover:text-accent-700">确定为世界观</button>
      ...
      <div v-if="sending" class="p-4 rounded-lg bg-white border border-neutral-200 max-w-[85%]">
        <p class="text-sm text-neutral-400">思考中...</p>
      </div>
    </div>

    <!-- Suggestions (after conclude) -->
    <div v-if="suggestions.length" class="card p-5 mb-6">
      <h2 class="text-sm font-medium text-neutral-600 mb-3">对话结论</h2>
      <ul class="space-y-3">
        <li v-for="(s, i) in suggestions" :key="i" class="flex items-start gap-2">
          <span class="badge bg-neutral-100 text-neutral-600 shrink-0 mt-0.5">{{ s.type }}</span>
      ...
      <p v-if="outlineGenerated" class="text-xs text-emerald-600 mt-1">总纲和卷纲已生成，<router-link :to="`/novels/${novelId}/outlines`" class="text-accent-600 underline">查看大纲</router-link></p>
    </div>
