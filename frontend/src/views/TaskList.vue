<template>
  <div class="max-w-6xl mx-auto px-6 py-10 animate-fade-up">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
      <div>
        <h1 class="heading-serif text-2xl tracking-tight">任务列表</h1>
        <p class="text-ink-400 text-sm mt-1">监控和管理小说生成任务的后台进度</p>
      </div>

      <!-- Filter Tabs -->
      <div class="flex items-center gap-3">
        <button @click="cleanupStaleTasks" class="text-xs font-medium text-ink-400 hover:text-red-500 border border-ink-200 rounded-lg px-3 py-1.5 transition-colors">
          清理过期任务
        </button>
        <div class="flex items-center bg-paper-50 p-1 rounded-xl border border-ink-200 shadow-sm">
        <button
          ...
          :class="[
            ...
            statusFilter === tab.value
              ? 'bg-paper-50 text-vermilion-600 shadow-sm border border-ink-100'
    ...
    <!-- Loading State -->
    <div v-if="loading && tasks.length === 0" class="text-center py-24 text-ink-400 flex flex-col items-center gap-3">
      <div class="w-8 h-8 border-2 border-vermilion-500 border-t-transparent rounded-full animate-spin"></div>
      <p class="text-xs font-medium tracking-wide">正在同步任务状态...</p>
    </div>

    <!-- Empty State -->
    <div v-else-if="filteredTasks.length === 0" class="text-center py-20 card flex flex-col items-center justify-center shadow-sm rounded-2xl border border-ink-100">
      <div class="w-12 h-12 rounded-full bg-paper-50 flex items-center justify-center text-ink-300 mb-4 text-xl">📝</div>
      <p class="text-ink-300 text-sm font-medium mb-4">没有找到相关生成任务</p>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        ...
        :key="task.task_id"
        class="card card-hover shine-on-hover animate-fade-up-stagger p-5 group flex flex-col justify-between hover:border-ink-300 hover:shadow-md transition-all duration-300 rounded-2xl"
      ...
      >
        <div>
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[11px] font-bold text-vermilion-600 bg-vermilion-50 px-2 py-0.5 rounded">
              {{ task.novel_type || '小说生成' }}
            </span>
          </div>

          <!-- Creative Idea Preview / Task ID -->
          <h3 class="font-bold text-ink-700 group-hover:text-vermilion-500 transition-colors mb-2 line-clamp-1 text-base">
            {{ task.idea ? truncateIdea(task.idea) : '无标题生成任务' }}
          </h3>

          <p class="text-[10px] text-ink-300 font-mono mb-4">{{ task.task_id }}</p>

          <!-- Idea Snippet -->
          <p class="text-xs text-ink-400 line-clamp-3 mb-6 leading-relaxed">
        ...
        <div class="border-t border-ink-100 pt-4 mt-auto">
          ...
          <div v-if="task.status === 'running' || task.status === 'pending'" class="mb-4">
            <div class="flex justify-between items-center text-xs font-semibold text-ink-600 mb-1.5">
              <span class="flex items-center gap-1.5">
                <span class="w-1.5 h-1.5 bg-vermilion-500 rounded-full animate-ping"></span>
                后台渲染中...
              </span>
              <span class="text-vermilion-500 font-mono">{{ task.progress_percentage }}%</span>
            </div>
            <!-- Progress Bar -->
            <div class="w-full bg-paper-100 rounded-full h-2 overflow-hidden">
              <div
                class="bg-vermilion-500 h-full rounded-full transition-all duration-300 progress-bar-striped animate-pulse"
          ...
          <div class="flex justify-between items-center">
            <p class="text-[10px] text-ink-300 font-medium">
            ...
            <div class="flex gap-3">
              ...
              <router-link
                ...
                class="text-xs font-bold text-vermilion-500 hover:text-vermilion-600 transition-colors"
              >
    ...
    <div v-if="total > limit" class="flex justify-center mt-10 gap-3">
      ...
      <span class="px-4 py-2 text-sm text-ink-400 font-medium">
