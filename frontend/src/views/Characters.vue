<template>
  <div class="max-w-3xl mx-auto px-6 py-10">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-xl font-bold text-ink-900">人物管理</h1>
      <router-link :to="`/novels/${novelId}`" class="btn-secondary text-sm">返回</router-link>
    </div>

    <!-- Add Character Form -->
    <div class="card p-5 mb-6">
      <h2 class="text-sm font-medium text-ink-600 mb-3">添加人物</h2>
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-2">
              <span class="font-bold text-ink-800">{{ char.name }}</span>
              <span v-if="char.role" class="badge bg-primary-50 text-primary-700">{{ char.role }}</span>
            </div>
            <p v-if="char.description" class="text-sm text-ink-600 mb-1">{{ char.description }}</p>
            <p v-if="char.personality" class="text-sm text-ink-500"><span class="text-ink-400">性格：</span>{{ char.personality }}</p>
            <p v-if="char.abilities" class="text-sm text-ink-500"><span class="text-ink-400">能力：</span>{{ char.abilities }}</p>
          </div>
          <button @click="deleteChar(char.id)" class="text-red-400 hover:text-red-600 text-sm ml-4">删除</button>
        </div>
      </div>
      <p v-if="characters.length === 0" class="text-center text-ink-400 py-8">暂无人物，请添加</p>
    </div>
  </div>
</template>
