<template>
  <div class="min-h-screen bg-paper-50 text-ink-600 relative overflow-hidden flex flex-col">
    <!-- 极淡纸面光晕，呼应水墨宣纸 -->
    <div class="absolute w-72 h-72 rounded-full bg-vermilion-500/5 blur-[100px] top-10 left-10 animate-pulse pointer-events-none"></div>
    <div class="absolute w-80 h-80 rounded-full bg-ink-400/5 blur-[120px] bottom-20 right-20 animate-pulse pointer-events-none" style="animation-duration: 6s;"></div>

    <!-- Header -->
    <header class="h-16 flex items-center justify-between px-6 border-b border-ink-100 backdrop-blur-md sticky top-0 z-40 bg-paper-50/85 select-none">
      <div class="flex items-center gap-2">
        <router-link to="/" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <span class="text-lg font-serif font-bold text-ink-800">xIaoShuo</span>
          <span class="text-[9px] bg-vermilion-50 text-vermilion-600 border border-vermilion-200 px-1.5 py-0.5 rounded-full font-medium">灵感向导</span>
        </router-link>
      </div>
      <div class="flex items-center gap-3">
        <router-link to="/" class="text-xs text-ink-400 hover:text-ink-700 transition-colors bg-paper-100 border border-ink-200 px-3 py-1.5 rounded-lg">
          退出向导
        </router-link>
      </div>
    </header>
    ...
    <main class="flex-1 max-w-4xl w-full mx-auto px-4 py-6 flex flex-col justify-between overflow-hidden relative z-10">
      ...
      >
        <div v-for="(msg, idx) in chatHistory" :key="idx" class="space-y-3">
          ...
          <div v-if="msg.sender === 'ai'" class="flex items-start gap-3">
            <div class="shrink-0 w-8 h-8 rounded-xl bg-ink-700 flex items-center justify-center shadow-md text-xs font-bold text-paper-50 select-none">
              AI
            </div>
            <div class="flex-1 space-y-3">
              <!-- Message Bubble -->
              <div class="bg-paper-100 border border-ink-200 rounded-2xl rounded-tl-none px-4 py-3 shadow-sm text-sm leading-relaxed max-w-[90%] text-ink-700">
              ...
              <div v-if="msg.step === currentStep && !msg.completed" class="flex flex-wrap gap-2 max-w-[90%] pt-1 animate-[fadeIn_0.3s_ease-out]">
                ...
                <template v-if="[0, 1, 2, 4].includes(currentStep) && currentOptions.length > 0">
                  <button
                    ...
                    type="button"
                    :class="currentStep === 2
                      ? 'block w-full text-left p-3 bg-paper-100 hover:bg-paper-200 border border-ink-200 hover:border-ink-300 rounded-xl text-xs leading-relaxed text-ink-600 transition-all duration-200 active:scale-[0.99]'
                      : 'px-3.5 py-2 bg-paper-50 hover:bg-paper-100 border border-ink-200 hover:border-vermilion-300 rounded-xl text-xs font-medium text-vermilion-600 transition-all duration-200 active:scale-[0.97]'"
                  ...
                  <div class="w-full space-y-3">
                      ...
                      <button
                        ...
                        :class="[
                          ...
                          selectedThemes.includes(theme)
                            ? 'bg-vermilion-500 border-vermilion-400 text-paper-50'
                            : 'bg-paper-100 border-ink-200 text-ink-500 hover:bg-paper-200'
                        ]"
                    ...
                    <div class="flex gap-2">
                      <input
                        ...
                        type="text"
                        class="bg-paper-50 border border-ink-200 rounded-lg text-xs px-3 py-1.5 text-ink-600 placeholder-ink-300 focus:outline-none focus:border-vermilion-400 flex-1"
                        placeholder="自定义主题标签..."
                ...
                <template v-if="currentStep === 5">
                  <div class="w-full bg-paper-100 border border-ink-200 rounded-2xl p-5 space-y-4">
                    <h3 class="text-sm font-serif font-bold text-ink-800 flex items-center gap-1.5">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-[11px]">
                      <div class="bg-paper-50 border border-ink-200 rounded-lg px-3 py-2">
                        <span class="text-ink-400">标题</span>
                        <p class="text-vermilion-600 font-medium mt-0.5">{{ collected.title || '—' }}</p>
                      </div>
                      <div class="bg-paper-50 border border-ink-200 rounded-lg px-3 py-2">
                        <span class="text-ink-400">类型</span>
                        <p class="text-ink-700 font-medium mt-0.5">{{ collected.genre || '—' }}</p>
                      </div>
                    </div>

                    <!-- LLM 生成的大纲（文本） -->
                    <div v-if="outlineText" class="space-y-1">
                      <label class="block text-[11px] text-ink-400 font-medium">AI 生成大纲（可在创建后于大纲编辑器中细化）</label>
                      <div class="w-full bg-paper-50 border border-ink-200 rounded-lg p-3 text-xs text-ink-600 leading-relaxed whitespace-pre-wrap max-h-72 overflow-y-auto">{{ outlineText }}</div>
                    </div>
                    <div v-else class="text-xs text-ink-400 bg-paper-50 border border-ink-200 rounded-lg p-3">
          ...
          <div v-if="msg.sender === 'user'" class="flex items-start justify-end gap-3">
            <div class="flex-grow flex justify-end">
              <div class="bg-vermilion-500 border border-vermilion-400 text-paper-50 rounded-2xl rounded-tr-none px-4 py-3 shadow-md text-sm max-w-[85%] leading-relaxed">
                <p>{{ msg.text }}</p>
              </div>
            </div>
            <div class="shrink-0 w-8 h-8 rounded-xl bg-ink-200 flex items-center justify-center text-xs font-bold text-ink-700 select-none">
              我
            </div>
          </div>
        </div>
        ...
        <div v-if="aiLoading" class="flex items-start gap-3 animate-pulse">
          ...
          <div class="bg-paper-100 border border-ink-200 rounded-2xl rounded-tl-none px-4 py-3.5 shadow-sm flex items-center gap-2 max-w-[170px]">
            <div class="w-3.5 h-3.5 border-2 border-vermilion-500 border-t-transparent rounded-full animate-spin"></div>
            <span class="text-xs text-ink-400">AI 正在构思...</span>
          </div>
        </div>
      </div>

      <!-- User input and settings panel (fixed at bottom) -->
      <div v-if="currentStep < 5" class="bg-paper-100 border border-ink-200 rounded-2xl p-4 space-y-4 shadow-sm">
        <!-- Settings (Only shows at appropriate steps) -->
        <div v-if="currentStep === 0" class="flex flex-wrap gap-4 text-xs text-ink-500 border-b border-ink-100 pb-3">
          <div class="flex items-center gap-2">
            <span>字数目标:</span>
            <select v-model="form.target_words" class="bg-paper-50 border border-ink-200 rounded px-2 py-1 text-ink-600 focus:outline-none focus:border-vermilion-400">
              <option :value="50000">5万字 (短篇)</option>
              <option :value="100000">10万字 (中篇)</option>
              <option :value="300000">30万字 (长篇)</option>
              <option :value="1000000">100万字 (长篇巨著)</option>
            </select>
          </div>
          <div class="flex items-center gap-2">
            <span>文风倾向:</span>
            <select v-model="form.writing_style" class="bg-paper-50 border border-ink-200 rounded px-2 py-1 text-ink-600 focus:outline-none focus:border-vermilion-400">
              <option value="轻松幽默">轻松幽默</option>
              <option value="热血燃向">热血燃向</option>
              <option value="细腻文艺">细腻文艺</option>
              <option value="史诗厚重">史诗厚重</option>
              <option value="悬疑紧张">悬疑紧张</option>
              <option value="古风典雅">古风典雅</option>
              <option value="现代白话">现代白话</option>
            </select>
          </div>
        </div>

        <!-- Progress Indicator -->
        <div class="flex justify-between items-center text-[10px] text-ink-400 px-1 select-none">
          <span>创意对话进度</span>
          <span>已完成 {{ Math.min(100, Math.round((currentStep / 5) * 100)) }}%</span>
        </div>
        <div class="w-full h-1 bg-ink-100 rounded-full overflow-hidden">
          <div
            class="h-full bg-gradient-to-r from-vermilion-500 to-ink-700 rounded-full transition-all duration-300"
        ...
        <form @submit.prevent="submitInput" class="flex gap-2">
          <input
            ...
            v-model="userInput"
            type="text"
            class="w-full px-4 py-2.5 bg-paper-50 text-ink-700 placeholder-ink-300 border border-ink-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-vermilion-200 focus:border-vermilion-400 transition-all text-sm animate-none"
          ...
          <button
            type="submit"
            class="btn-primary px-5 py-2.5 flex items-center justify-center shrink-0 rounded-xl"
      ...
      <div v-if="projectCreating" class="fixed inset-0 z-50 bg-ink-900/40 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
        <div class="w-10 h-10 border-4 border-vermilion-500 border-t-transparent rounded-full animate-spin"></div>
        <p class="text-sm font-medium text-ink-600">正在为您创建小说项目...</p>
      </div>
    </main>
  </div>
</template>
