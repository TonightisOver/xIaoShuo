  ...
  <div class="space-y-6">
    ...
    <div v-if="unassignedChapters.length" class="mt-6">
      ...
      <div class="bg-white rounded-xl border border-neutral-200 shadow-sm divide-y divide-neutral-100">
        <div v-for="ch in unassignedChapters" :key="ch.id" class="flex items-center justify-between px-6 py-3.5 hover:bg-neutral-50 transition-colors group">
          ...
          <div class="flex items-center gap-3 shrink-0">
            ...
            <button @click="$emit('confirm-delete', ch)" class="opacity-0 group-hover:opacity-100 text-neutral-300 hover:text-red-500 transition-all p-1 rounded" title="删除章节">
    ...
    <Teleport to="body">
      <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-xl shadow-xl p-6 w-80 mx-4 border border-neutral-200">
          ...
          <p class="text-sm text-neutral-500 mb-5">确定要删除「第{{ deleteTarget.chapter_number }}章：{{ deleteTarget.title }}」吗？此操作不可撤销。</p>
          <div class="flex gap-3 justify-end">
            <button @click="$emit('cancel-delete')" class="px-4 py-2 text-sm font-medium text-neutral-600 hover:text-neutral-800 rounded-lg hover:bg-neutral-100 transition-colors">取消</button>
            <button @click="$emit('do-delete')" class="px-4 py-2 text-sm font-medium text-white bg-red-500 hover:bg-red-600 rounded-lg transition-colors">删除</button>
          </div>
        </div>
      </div>
    </Teleport>
