<template>
  <div class="space-y-6">
    <div v-if="loading" class="flex flex-col items-center justify-center py-20 space-y-3">
      ...
      <p class="text-xs text-neutral-400">正在生成深度质量审计报告...</p>
    </div>
    ...
    <div v-else-if="!report || !report.volume_reports || report.volume_reports.length === 0" class="card p-8 text-center text-neutral-400 text-sm">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div v-for="(val, key) in report.overall_avg_scores" :key="key" class="card p-4 bg-[#1a1c2a] border border-[#27293d] flex flex-col justify-between h-28">
          <div class="text-[10px] text-neutral-400 uppercase font-bold tracking-wider">{{ dimensionLabel(key) }}</div>
          <div class="flex items-baseline justify-between mt-2">
            <span class="text-3xl font-extrabold" :class="scoreColorClass(val)">{{ (val * 10).toFixed(1) }}</span>
            <span class="text-[10px] text-neutral-500 font-medium">/ 10.0</span>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 card p-6 bg-[#151722] border border-[#27293d] space-y-4">
          ...
          <div v-else class="space-y-2">
            <div v-for="(warn, index) in allWarnings" :key="index" class="text-xs text-amber-300 bg-amber-950/20 border border-amber-900/50 rounded-xl p-3 flex items-start gap-2.5 leading-relaxed">
              <span class="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 shrink-0"></span>
              <div>
                <div class="font-bold text-neutral-200">异常检测：{{ warn }}</div>
                <div class="text-[11px] text-neutral-400 mt-0.5">建议：{{ getSuggestion(warn) }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card p-6 bg-[#151722] border border-[#27293d] space-y-4">
          <h3 class="text-sm font-bold text-neutral-200">段落状态检测</h3>

          <div class="space-y-4">
            <div>
              <span class="text-xs text-neutral-400 font-semibold block mb-1.5">灌水嫌疑章节 (Filler chapters)</span>
              <div v-if="fillerChapters.length === 0" class="text-xs text-neutral-500 italic">暂无灌水嫌疑</div>
            ...
            <div class="border-t border-[#27293d] pt-3">
              <span class="text-xs text-neutral-400 font-semibold block mb-1.5">情节推进缓慢章节 (Stalled chapters)</span>
              <div v-if="stalledChapters.length === 0" class="text-xs text-neutral-500 italic">推进节奏良好</div>
              <div v-else class="flex flex-wrap gap-1.5">
      ...
      <div class="space-y-4">
        <h3 class="text-sm font-bold text-neutral-200">分卷趋势深度追踪</h3>
        <div v-for="vol in report.volume_reports" :key="vol.volume_number" class="card p-6 bg-[#151722] border border-[#27293d] space-y-4">
          <div class="flex justify-between items-center pb-2 border-b border-[#27293d]">
            <h4 class="font-extrabold text-neutral-100 text-sm">
              {{ vol.volume_number === 0 ? '未分卷章节质量快照' : `第 ${vol.volume_number} 卷质量快照` }}
            </h4>
            <span class="text-xs text-neutral-400 font-medium">共 {{ vol.chapter_count }} 章 · {{ (vol.total_word_count / 1000).toFixed(1) }}k 字</span>
          </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
            <div v-for="(score, dim) in vol.avg_scores" :key="dim" class="bg-[#0f1015] border border-[#27293d] rounded-xl p-2.5 text-center">
              <div class="text-[9px] text-neutral-500 uppercase font-bold truncate">{{ dimensionLabel(dim) }}</div>
