<template>
  <div class="card p-5 mt-6">
    <div class="flex items-center justify-between mb-4">
      <h3 class="font-medium text-ink-800 text-sm">读者视角模拟</h3>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      <label v-for="p in personas" :key="p.id"
        ...
        :class="selectedPersonas.includes(p.id) ? 'border-accent-300 bg-accent-50 text-accent-700' : 'border-ink-200 text-ink-600 hover:bg-ink-50'">
    ...
    <div v-if="simulating" class="text-center py-8">
      ...
      <p class="text-xs text-ink-500 mt-2">正在模拟读者反馈，请稍候...</p>
    </div>

    <!-- Results -->
    <div v-if="currentResults.length > 0" class="space-y-3">
      <div v-for="r in currentResults" :key="r.persona_id"
        class="border border-ink-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-2">
          <span class="font-medium text-sm text-ink-800">{{ r.persona_name }}</span>
        ...
        <template v-if="!r.error">
          ...
          <div v-if="r.satisfaction_points?.length" class="mb-1.5">
            <span class="text-[11px] text-ink-500">亮点：</span>
          ...
          <div v-if="r.pain_points?.length" class="mb-2">
            <span class="text-[11px] text-ink-500">不足：</span>
            <span v-for="(s, i) in r.pain_points" :key="i" class="text-xs text-red-700 bg-red-50 px-1.5 py-0.5 rounded mr-1">{{ s }}</span>
          </div>

          <!-- Overall comment -->
          <p class="text-xs text-ink-600 italic border-l-2 border-ink-200 pl-2 mt-2">
            "{{ r.overall_comment }}"
          </p>
        </template>
    ...
    <!-- History -->
    <div v-if="history.length > 0" class="mt-4 border-t border-ink-200 pt-3">
      <button @click="showHistory = !showHistory" class="text-xs text-ink-500 hover:text-ink-700 font-medium">
      ...
      <div v-if="showHistory" class="mt-2 space-y-1">
        <div v-for="h in history" :key="h.id" @click="loadSimulation(h.id)"
          class="text-xs text-ink-600 px-2 py-1.5 rounded hover:bg-ink-50 cursor-pointer flex justify-between">
          <span>{{ h.personas_used?.length || 0 }}个视角 · {{ h.duration_ms ? (h.duration_ms / 1000).toFixed(1) + 's' : '...' }}</span>
          <span class="text-ink-400">{{ formatTime(h.created_at) }}</span>
        </div>
      </div>
    </div>
  </div>
