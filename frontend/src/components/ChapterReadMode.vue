<template>
  <div ref="readContainer" class="fixed inset-0 z-50 overflow-y-auto transition-all duration-300 custom-scrollbar-read animate-fade-up" :class="[themeClasses[activeTheme]]">
    <div class="max-w-3xl mx-auto px-6 py-20 relative min-h-screen flex flex-col justify-between">
      <div>
        ...
        <h1 class="text-3xl md:text-4xl font-extrabold mb-10 text-center tracking-tight heading-serif" :class="[activeFont === 'serif' ? 'font-serif' : 'font-sans']">
    ...
    <div class="fixed right-6 bottom-10 z-50 flex flex-col items-end gap-3">
      <div v-if="showSettings" class="bg-paper-50 rounded-xl border border-ink-200 shadow-lg p-5 flex flex-col gap-4 w-72 animate-fade-in" :class="[activeTheme === 'dark' ? 'bg-neutral-900 text-slate-200 border-neutral-700' : '']">
        <div class="flex items-center justify-between border-b pb-2" :class="[activeTheme === 'dark' ? 'border-neutral-700' : 'border-ink-200']">
          <span class="text-xs font-bold tracking-wider text-ink-600">排版个性化设置</span>
          <button type="button" aria-label="关闭排版设置" @click="$emit('update:show-settings', false)" class="text-xs text-ink-400 hover:text-ink-600">✕ 关闭</button>
        </div>
        <div class="flex flex-col gap-1.5">
          <span class="text-[11px] text-ink-500 font-semibold">字号大小</span>
          <div class="flex items-center justify-between gap-2">
            <button type="button" aria-label="减小字号" @click="$emit('adjust-font-size', -2)" class="flex-1 py-1.5 rounded-lg border text-center font-bold text-sm transition-all border-ink-200 hover:bg-paper-100">A -</button>
            <span class="text-xs font-mono font-bold w-12 text-center text-ink-600">{{ fontSize }}px</span>
            <button type="button" aria-label="增大字号" @click="$emit('adjust-font-size', 2)" class="flex-1 py-1.5 rounded-lg border text-center font-bold text-sm transition-all border-ink-200 hover:bg-paper-100">A +</button>
          </div>
        </div>
        <div class="flex flex-col gap-1.5">
          <span class="text-[11px] text-ink-500 font-semibold">阅读字体</span>
          <div class="flex gap-2">
            <button type="button" aria-label="使用衬线字体" @click="$emit('update:active-font', 'serif')" class="flex-1 py-1.5 rounded-lg border text-xs transition-all" :class="[activeFont === 'serif' ? 'border-vermilion-500 bg-vermilion-50 text-vermilion-600 font-bold' : 'border-ink-200']">宋体 / 衬线</button>
            <button type="button" aria-label="使用无衬线字体" @click="$emit('update:active-font', 'sans')" class="flex-1 py-1.5 rounded-lg border text-xs transition-all" :class="[activeFont === 'sans' ? 'border-vermilion-500 bg-vermilion-50 text-vermilion-600 font-bold' : 'border-ink-200']">系统 / 无衬线</button>
          </div>
        </div>
        <div class="flex flex-col gap-1.5">
          <span class="text-[11px] text-ink-500 font-semibold">背景配色</span>
      ...
      <button type="button" aria-label="打开目录" @click="showTOC = true" class="w-12 h-12 rounded-full shadow-lg flex items-center justify-center text-xl transition-all active:scale-95 bg-paper-50 text-ink-600 border border-ink-200 hover:bg-paper-100" :class="[activeTheme === 'dark' ? 'bg-neutral-900 text-slate-200 border-neutral-700 hover:bg-neutral-800' : '']">☰</button>
      <button type="button" aria-label="切换排版设置" @click="$emit('update:show-settings', !showSettings)" class="btn-primary w-12 h-12 rounded-full shadow-lg flex items-center justify-center text-xl transition-all active:scale-95">⚙</button>
    </div>
  </div>
</template>
