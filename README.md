...
8. **阶段化创作控制台** — 10 阶段可视化控制面板（`/novels/:id/studio`）：查看/编辑/锁定/审批/局部重生成任意阶段产物，依赖感知的影响范围预测，乐观锁并发保护（409），操作全程审计；三模式（自动/辅助/手动）并存，保留一键全自动生成
...
| 创作控制台 | `/api/v1/projects/{id}/creative-control` | 阶段/产物查看编辑、锁定审批、影响分析、局部重生成、版本回滚、操作审计 |
