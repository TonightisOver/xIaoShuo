...
【重要规则】
1. 【字数控制】本章目标 {target_words} 字（允许范围 {target_words_min}~{target_words_max} 字）。严禁超过 {target_words_max} 字。如果情节写完但字数不足，补充细节描写；如果接近上限，尽快收束本章。
...
【最终检查】在输出前确认：(1) 字数在 {target_words_min}~{target_words_max} 范围内；(2) 开头与上一章结尾衔接自然。
