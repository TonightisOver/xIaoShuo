...
class MemoryCuratorAgent:
    ...
    async def curate(
        ...
        logger.info("memory_curator_curating", chapter=chapter_num)
        ...
        if self.task_id and recorder is not None:
            journal = await recorder.create_entry(
                ...
                input_summary={
                    "chapter": chapter_num,
                    "kg_length": len(raw_kg_context or ""),
                    "bible_length": len(story_bible_context or ""),
                },
            )
        ...
        try:
            ...
            logger.info("memory_curator_completed", chapter=chapter_num)

            if journal:
                await recorder.complete_entry(
                    entry_id=journal.id,
                    status="success",
                    output_summary={"memo_length": len(memo)},
